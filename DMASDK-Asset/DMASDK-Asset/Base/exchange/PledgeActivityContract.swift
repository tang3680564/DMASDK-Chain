...
class PledgeActivityContract : NSObject {
    ...
    ///   - getGasFee: 估算这次操作所需要的gasfee , true : 进行估算,不进行这次操作, false : 不进行估算,进行这次操作
    /// - Returns: hash
    func transfer(privateKey:String,contractAddress:String,tokenId:String, weiValue: String ,gasLimit:String,gasPrice:String,getGasFee : Bool = false) -> ContractResult {
